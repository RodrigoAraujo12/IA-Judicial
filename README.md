...
| **Serviço para escritórios** | **no ar desde 23/09/2026** | `TRIAGEM_MODO=servico`: login obrigatório, um arquivo de casos por escritório, corpus compartilhado e somente leitura — ver [Modo serviço](#modo-serviço-login-e-escritórios). Quem abriu qual caso fica registrado — ver [Registro de acesso](#registro-de-acesso). A instalação para servidor está em [`IMPLANTACAO.md`](IMPLANTACAO.md) e **foi executada em 23/09/2026**, num servidor do plano grátis da Oracle: HTTPS válido, backup agendado e registro de acesso funcionando. Só casos fictícios até haver contrato de LGPD com o escritório. Reabre a premissa local do `ENTREGA.md` — ver [Rumo](#rumo-um-serviço-para-escritórios). |
